...
config = dotenv.dotenv_values("./databases/secret files/config.env")
...
@group1.command(name="setup", description="Configures the bot's spawn channel!")
@app_commands.checks.has_permissions(administrator=True)
async def setup_config(interaction, channel: TextChannel):
    with open("./databases/channel_setup.json", "r") as file:
        spawn_channel = json.load(file)
...
@group1.command(name="disable", description="Disables spawning in a server.")
@app_commands.checks.has_permissions(administrator=True)
async def disable_spawn(interaction):
    with open("./databases/channel_setup.json", "r") as file:
        spawn_channel = json.load(file)
...
@disable_spawn.error
async def disable_spawn_error(interaction, error):
    with open("./databases/channel_setup.json", "r") as file:
        spawn_channel = json.load(file)
    with open("./databases/channel_setup.json", "w") as file:
        json.dump(spawn_channel, file)
...
@group2.command(name="list", description="See what Altballs you or somebody has!")
async def balls_list(interaction, user:Member=None, sort:str=None):
    if user == None:
...
@group2.command(name="codex", description="Show your current completion of the Alterdex!")
async def completion(interaction):
    update_user(interaction.user)
    countryballs = load("./databases/countryball_list.json")
    user_completion = load("./databases/user_data.json")
    ball_emoji = load("./databases/emoji_ids.json")
...
@group2.command(name="recent", description="See your last caught Altball!")
async def recently_caught(interaction):
    update_user(interaction.user)
    user_completion = load("./databases/user_data.json")
    catch_dates = load("./databases/catch_date.json")
...
@group2.command(name="info", description="Shows you the info of a countryball!")
async def ball_info(interaction, countryball: str):
    update_user(interaction.user)
    user_completion = load("./databases/user_data.json")
    catch_dates = load("./databases/catch_date.json")
...
@group2.command(name="favorite", description="Set an Altball as your favorite!")
async def favorite_set(interaction, countryball: str):
    update_user(interaction.user)
    favorites = load("./databases/favorites_list.json")
    ball_emoji = load("./databases/emoji_ids.json")
    inList = False
    for i in range(len(favorites[str(interaction.user.id)])):
        if (favorites[str(interaction.user.id)][i] == countryball):
            inList = True
    if (inList):
        await interaction.response.send_message(f'''{ball_emoji[countryball]} **{countryball}** is already an Altball. Choose something else''', ephemeral=True)
    else:
        with open("./databases/favorites_list.json", "r") as file:
            favorites = json.load(file)
...
@group2.command(name="donate", description="Gift a user one of your Altballs!")
async def give_ball(interaction, user: Member, countryball: str):
...
@group2.command(name="count", description="Count how many Altballs you have!")
async def ballCount(interaction, user:Member=None):
    user_completion = load("./databases/user_data.json")
    if user == None:
        update_user(interaction.user)
...
@group2.command(name="rarity", description="Shows the rarities of all Altballs.")
async def rarityList(interaction):
    update_user(interaction.user)
    countryballs = load("./databases/countryball_list.json")
    user_completion = load("./databases/user_data.json")
    ball_emoji = load("./databases/emoji_ids.json")
